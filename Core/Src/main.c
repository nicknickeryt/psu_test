...
uint32_t currentMiliAmps = 0.0f;
uint32_t powerWatts = 0.0f;

void psuBootstrap() {
    while (DPM_Ports[USBPD_PORT_0].DPM_NumberOfRcvSRCPDO == 0) {
        USBPD_DPM_Run();

        uint32_t num_pdos = DPM_Ports[USBPD_PORT_0].DPM_NumberOfRcvSRCPDO;
        ...
        for (uint32_t i = 0; i < num_pdos; i++) {
            ...
            switch (pdo.GenericPDO.PowerObject) {
                case USBPD_CORE_PDO_TYPE_FIXED: {
                    ...
                    voltageVolts = (float)fixed.VoltageIn50mVunits *
                                   0.05f;  // przelicz na V
                    currentMiliAmps = (float)fixed.MaxCurrentIn10mAunits *
                                      10.0f;  // przelicz na A
...
int main(void) {
    ...
    send_string("USB PD Snk1M1 Example Start\r\n");
...
static void MX_DMA_Init(void) {
    ...
    NVIC_SetPriority(DMA2_Channel2_IRQn,
                     NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 0, 0));
